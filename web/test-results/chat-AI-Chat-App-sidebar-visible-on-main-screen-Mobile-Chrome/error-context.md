...
- Name: chat.spec.ts >> AI Chat App >> sidebar visible on main screen
- Location: tests/chat.spec.ts:71:3
...
> 78  |     expect(sidebarBox).not.toBeNull();
  ...
  171 |     const during = await page.screenshot();
  172 | 
  173 |     expect(before.length).not.toBe(during.length);
  174 |   });
  175 | 
  176 |   test('conversations sidebar loads', async ({ page }) => {
  177 |     await apiLogin(page);
  178 |     await page.screenshot({ path: 'tests/screenshots/sidebar-state.png', fullPage: true });
```